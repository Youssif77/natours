...
exports.forgetPassword = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    await new Email(user, resetURL).snedPasswordReset();

    res
      .status(200)
      .json({ status: 'success', message: 'Token snet to email!' });
  } catch (err) {
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  if (!user) return next(new AppError('Token is invaild or has expired', 400));
  ...
  // 3) Update changedPasswordAt property for the user (IN THE MODEL)
